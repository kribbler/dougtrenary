...
class YouTubeResponsive extends \WP_Widget {
        ...
        ?><script type="text/javascript">
                    function AI_responsive_widget() {
                        jQuery('iframe.StefanoAI-youtube-responsive').each(function () {
                            ...
                            var height = jQuery(this).css('max-height').replace(/px/, '');
    ...
    static function makeEmbedUrl($params) {
        ...
        if (!empty($idvideo)) {
            ...
            $maxh = !empty($params['maxh']) ? 'max-height:' . intval($params['maxh']) . 'px;' : '';

            @$id = ++$youtube_id;

            $track = !empty($params['track']) ? $and . "enablejsapi=1" . $and . "playerapiid=$id" : "";

            @$urlembed = "<iframe id='$id' class='StefanoAI-youtube-responsive $class' width='160' height='90' src='$url$idvideo?$idlist$autohide$autoplay$cc_load$cc_lang$color$controls$disablekb$end$fs$iv_load_policy$loop$modestbranding$rel$showinfo$start$theme$quality$wmode$track' frameborder='0' $allowfullscreen style='$maxw$maxh$style'></iframe>";
            if (!empty($params['track']) || 1==1) {
                YouTubeResponsive::$footer.=<<<SCRIPT
                var player_$id;
                console.log('here on youtube-widget-responsive');
                        ...
                        'onStateChange': function () {
                            ...
                            switch (player_$id.getPlayerState()) {
                                ...
                                case 0:
                                    ...
                                    log_user_video(d.video_id, '1');
                                    break;
                                case 1:
                                    //playing
                                    log_user_video(d.video_id, '0');
                                    //StefanoAI_trackYoutubeVideo('Playing', d.title + " | " + d.video_id + " (" + d.author + ")");
                                    break;
                                case 2:
                                    //paused
                                    //log_user_video(d.video_id, '2');
...
//save widget settings
        ...
        $instance['maxh'] = !empty($new_instance['maxh']) ? $new_instance['maxh'] : '';
    ...
    function form($instance) {
        ...
        $maxh = !empty($instance['maxh']) ? $instance['maxh'] : '';
